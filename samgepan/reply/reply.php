...
<div class="rp_wrap">
    ...
    <?php
        ...
        $sql = "select * from reply_table_sam where rt_isuse='y' order by rt_date desc limit {$limitTo},{$limit};";
                ...
                if($count>0){
                    for($i=0; $i<$count; $i++){
                        ...
                        if($rt_isadmin=='n'){
                            ...
                            echo "<p class='rp_name rp_name_admin'>관리자</p>";
                        }
                        
                        echo "</div>";
                        echo "<div class='rp_text'>".htmlspecialchars($rt_text)."</div>";
    ...
    <ul class="rp_paging">
        ...
        $sql = "select * from reply_table_sam where rt_isuse='y'"; //전체 카운트
        if($sql_result = mysql_query($sql, $conn)){
